...
def dilshuknagar_teacher_dashboard():
    st.title("Dilshuknagar Teacher Dashboard")
    ...
    if page == "Evaluation":
        ...
        # Fetch Subjects for Dilshuknagar
        subjects_df = load_data(f"SELECT id, name FROM Subjects WHERE branch_id = {branch_id}")

        if subjects_df.empty:
            st.warning("No subjects available for Dilshuknagar. Please add subjects using branch admin.")
        else:
